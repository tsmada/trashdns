...
pub async fn handle_cname_query(
    ...
    if let Some(query) = query_message.queries().first() {
        if query.query_type() == RecordType::CNAME {
            ...
            let results = resolver.resolve(&query.name().to_ascii(), RecordType::CNAME).await?;
            for result in results {
                let cname = Name::from_utf8(result)?;
